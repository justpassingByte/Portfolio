...
export const ProjectGallery: React.FC<ProjectGalleryProps> = ({ paths, title }) => {
  ...
  const getCaption = (path: string, index: number) => {
    ...
    if (filename.includes('chayfood')) return 'Farm-to-Table Nutrition Engine & Meal Subscription Dashboard';
          ...
          {paths.map((path, idx) => {
            ...
            return (
              <button
                ...
                className={`relative aspect-[16/10] w-24 sm:w-28 shrink-0 rounded-xl overflow-hidden border transition-all duration-200 bg-[#0F1424] ${
                  isActive
